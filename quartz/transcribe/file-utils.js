...
export class FileUtils {
     ...
     */
    static async validateInputFile(filePath) {
        try {
     ...
     */
    static async ensureOutputDirectory(outputPath) {
     ...
     */
    static async handleExistingOutputFile(outputPath, overwrite = false) {
        try {
     ...
     */
    static async cleanupFiles(filePaths, verbose = false) {
        const cleanedFiles = [];
        const failedFiles = [];
     ...
     */
    static async createTempDirectory(prefix = "quartz-transcribe") {
     ...
     */
    static generateSafeFilename(sessionNumber, date, suffix = "transcription") {
     ...
     */
    static estimateRequiredSpace(inputFileSize) {
        // Rough estimate:
...
FileUtils.SUPPORTED_EXTENSIONS = [".mkv"];
FileUtils.MAX_FILE_SIZE = 2 * 1024 * 1024 * 1024; // 2GB
