import { TranscriptionResult, TranscriptionSegment } from "./whisper-client.js"
import { ParsedSegment, CharacterParser } from "./character-parser.js"
...
export interface FormattingOptions {
  ...
  includeConfidenceMarkers?: boolean
  groupBySpeaker?: boolean
  markLowConfidence?: boolean
  confidenceThreshold?: number
  includeProcessingNotes?: boolean
  disableSpeakerIdentification?: boolean
}
...
export class SessionFormatter {
  ...
  formatTranscription(
    ...
    if (opts.disableSpeakerIdentification) {
      ...
      const parsedSegments = transcriptionResult.segments.map(segment =>
        this.parser.parseSegment(segment)
      )
  ...
  private getDefaultOptions(options: FormattingOptions): Required<FormattingOptions> {
    return {
  ...
  private generateFrontmatter(metadata: SessionMetadata): string {
    const frontmatter = [
      "---",
  ...
  private generateHeader(metadata: SessionMetadata): string {
  ...
  private renderProcessingNotes(notes: string[]): string {
  ...
  private generateTranscriptionContent(segments: ParsedSegment[], options: Required<FormattingOptions>): string {
  ...
  private formatSegment(segment: ParsedSegment, options: Required<FormattingOptions>): string {
    let segmentText = ""
  ...
  private generateSimpleTranscription(text: string, options: Required<FormattingOptions>): string {
    let content = "## Transcription\n\n"
  ...
  private generatePlainTranscription(text: string, options: Required<FormattingOptions>): string {
    let content = "## Transcription\n\n"
  ...
  private generatePlainTranscriptionWithTimestamps(segments: TranscriptionSegment[], options: Required<FormattingOptions>): string {
    let content = "## Transcription\n\n"
    let currentTimePeriod = ""
  ...
  private generateFooter(metadata: SessionMetadata, options: Required<FormattingOptions>): string {
    let footer = "---\n\n"
  ...
  private getTimePeriod(startTimeSeconds: number): string {
  ...
  private getQualityIcon(quality: "good" | "fair" | "poor"): string {
    switch (quality) {
      case "good": return "🟢"
      case "fair": return "🟡"
      case "poor": return "🔴"
  ...
  generateFilename(sessionNumber?: number, date?: string): string {
    if (sessionNumber) {
  ...
  assessAudioQuality(segments: ParsedSegment[]): "good" | "fair" | "poor" {
