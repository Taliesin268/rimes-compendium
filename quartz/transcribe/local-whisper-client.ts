...
export class LocalWhisperClient {
  ...
  async installWhisper(): Promise<boolean> {
  ...
  private getDefaultOptions(options: LocalWhisperOptions): Required<LocalWhisperOptions> {
    return {
  ...
  private buildWhisperArgs(
    inputPath: string,
    outputPath: string,
    options: Required<LocalWhisperOptions>
  ): string[] {
    const args = [
      "-m", "whisper",
      inputPath,
  ...
  private async runWhisperCommand(args: string[], verbose: boolean): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      if (!verbose) {
        ...
        process.stdout.on("data", (data) => {
          ...
          if (output.includes("%") || output.includes("Processing")) {
            (process.stdout as any).write(`\r${chalk.blue("Progress:")} ${output.trim()}`)
          }
        })
      }
      ...
      process.on("close", (code) => {
        if (code === 0) {
          if (!verbose) console.log() // New line after progress
          resolve()
        } else {
  ...
  private buildContextualPrompt(userPrompt?: string, previousResults: TranscriptionResult[] = []): string {
  ...
  getModelSizes(): Record<string, string> {
    return {
      "tiny": "~39 MB",
      "base": "~74 MB",
      "small": "~244 MB",
  ...
  getModelAccuracy(): Record<string, string> {
    return {
