...
export interface AudioProcessingOptions {
  enhanceDiscord?: boolean
  chunkSize?: string // e.g., "10m" for 10 minutes
...
export class AudioProcessor {
  ...
  async validateMkvFile(filePath: string): Promise<boolean> {
    try {
  ...
  async extractAudioFromMkv(
    ...
    const outputPath = path.join(
      this.tempDir,
      `extracted_audio_${Date.now()}.${outputFormat}`,
    )
  ...
  private applyDiscordEnhancements(command: ffmpeg.FfmpegCommand): ffmpeg.FfmpegCommand {
    return command
      .audioFilters([
  ...
  async splitAudioIntoChunks(
    ...
    console.log(
      chalk.blue(`Splitting audio into ${Math.ceil(totalDuration / chunkDuration)} chunks...`),
    )
    ...
    for (let start = 0; start < totalDuration; start += chunkDuration) {
      ...
      const chunkPath = path.join(
        this.tempDir,
        `chunk_${chunkIndex}_${Date.now()}.wav`,
      )
